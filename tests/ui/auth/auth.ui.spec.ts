...
test.describe("Autoryzacja i Profil", () => {
  test("TC-AUTH-01: Logowanie poprawnymi danymi (Happy Path)", async ({
    page,
    context,
  }) => {
    ...
    await test.step("Weryfikacja ciasteczek sesyjnych", async () => {
      ...
      const jwtCookie = cookies.find(
        (cookie) => cookie.name === "jwt_zuvoria_v1",
      );
      const refreshCookie = cookies.find(
        (cookie) => cookie.name === "refresh_zuvoria_v1",
      );
  ...
  test("TC-AUTH-02: Próba logowania z błędnym hasłem (Negative Test)", async ({
    page,
    context,
  }) => {
    const loginPage = new LoginPage(page);
