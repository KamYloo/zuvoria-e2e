...
test.describe('Odtwarzacz | REQ-PLAYER-01', () => {
  test('TC-PLAYER-01: Uruchomienie odtwarzacza i weryfikacja startu HLS po kliknięciu aktywnego utworu', async ({ page }) => {
    ...
    await test.step('Kliknij utwór, który zmienia aktualny wybór odtwarzania', async () => {
      const manifestResponsePromise = page.waitForResponse(
        (response) => {
          ...
          return /\.m3u8(\?|$)|manifest/i.test(url) && method === 'GET' && response.status() === 200;
        },
        { timeout: 15000 }
      );
